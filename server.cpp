...
static int32_t handle_accept(int fd, Conn::Proto proto) {
    // accept
...
int main() {
    ...
    while (true) {
        ...
        struct pollfd pfd_binary = {fd_binary, POLLIN, 0};
        struct pollfd pfd_resp   = {fd_resp,   POLLIN, 0};
        poll_args.push_back(pfd_binary);
        poll_args.push_back(pfd_resp);
        ...
        if (poll_args[0].revents) {
            handle_accept(fd_binary, Conn::PROTO_BINARY);
        }
        if (poll_args[1].revents) {
            handle_accept(fd_resp, Conn::PROTO_RESP);
        }

        // handle connection sockets
        for (size_t i = 2; i < poll_args.size(); ++i) { // note: skip the 1st
